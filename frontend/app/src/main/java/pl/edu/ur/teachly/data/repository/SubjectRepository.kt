...
class SubjectRepository(private val api: SubjectApiService) {
    suspend fun getAllSubjects(): Result<List<SubjectResponse>> = try {
    ...
    suspend fun getAllCategories(): Result<List<SubjectCategoryResponse>> = try {
        val response = api.getAllCategories()
        if (response.isSuccessful) {
            Result.success(response.body()!!)
        } else {
            Result.failure(Exception("Błąd pobierania kategorii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }

    suspend fun addSubject(request: SubjectRequest): Result<SubjectResponse> = try {
    ...
    suspend fun updateSubject(id: Int, request: SubjectRequest): Result<SubjectResponse> = try {
        val response = api.updateSubject(id, request)
        if (response.isSuccessful) {
            Result.success(response.body()!!)
        } else {
            Result.failure(Exception("Błąd aktualizacji przedmiotu"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }

    suspend fun deleteSubject(id: Int): Result<Unit> = try {
    ...
    suspend fun addCategory(request: SubjectCategoryRequest): Result<SubjectCategoryResponse> = try {
        val response = api.addCategory(request)
        if (response.isSuccessful) {
            Result.success(response.body()!!)
        } else {
            Result.failure(Exception("Błąd dodawania kategorii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }

    suspend fun updateCategory(id: Int, request: SubjectCategoryRequest): Result<SubjectCategoryResponse> = try {
        val response = api.updateCategory(id, request)
        if (response.isSuccessful) {
            Result.success(response.body()!!)
        } else {
            Result.failure(Exception("Błąd aktualizacji kategorii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }
    ...
    suspend fun deleteCategory(id: Int): Result<Unit> = try {
        ...
        if (response.isSuccessful) {
            ...
            Result.failure(Exception("Błąd usuwania kategorii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }
