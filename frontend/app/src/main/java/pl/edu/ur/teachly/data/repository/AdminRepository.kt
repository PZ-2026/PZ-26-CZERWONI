...
class AdminRepository(private val api: AdminApiService) {
    ...
    suspend fun getStats(): Result<AdminStatsResponse> = try {
        val response = api.getStats()
        if (response.isSuccessful) {
            Result.success(response.body()!!)
        } else {
            Result.failure(Exception("Błąd pobierania statystyk"))
        }
    } catch (e: Exception) {
    ...
    suspend fun getAllReviews(): Result<List<ReviewResponse>> = try {
        ...
        if (response.isSuccessful) {
            ...
            Result.failure(Exception("Błąd pobierania opinii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }
    ...
    suspend fun deleteReview(reviewId: Int): Result<Unit> = try {
        ...
        if (response.isSuccessful) {
            ...
            Result.failure(Exception("Błąd usunięcia opinii"))
        }
    } catch (e: Exception) {
        Result.failure(Exception("Brak połączenia z serwerem"))
    }
