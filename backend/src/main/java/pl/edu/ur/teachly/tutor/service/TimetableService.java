...
public class TimetableService {
    ...
    public List<TimetableDayResponse> getTimetable(Integer tutorId, LocalDate fromDate, LocalDate toDate, Integer currentStudentId) {
        ...
        List<Lesson> lessons = lessonRepository.findByTutor_UserIdAndLessonDateBetween(tutorId, fromDate, toDate);
        ...
        for (LocalDate date = fromDate; !date.isAfter(toDate); date = date.plusDays(1)) {
            ...
            boolean isHoliday = holidays.stream().anyMatch(h -> h.getHolidayDate().equals(currentDate));
            if (isHoliday) {
                timetable.add(new TimetableDayResponse(currentDate, Collections.emptyList()));
                continue;
            }

            Optional<TutorAvailabilityOverride> overrideOpt = overrides.stream()
            ...
            if (overrideOpt.isPresent()) {
                ...
                List<TutorAvailabilityRecurring> dayRecurrings = recurrings.stream()
                        .filter(r -> r.getDayOfWeek() == dayOfWeek)
                        .filter(r -> r.getDateTo() == null || !r.getDateTo().isBefore(currentDate))
                        .toList();
            ...
            freeBlocks = freeBlocks.stream()
                    .filter(b -> java.time.Duration.between(b.getTimeFrom(), b.getTimeTo()).toMinutes() >= 30)
    ...
    private List<TimeSlot> subtractLesson(List<TimeSlot> blocks, LocalTime lessonStart, LocalTime lessonEnd) {
        List<TimeSlot> updatedBlocks = new ArrayList<>();
        for (TimeSlot block : blocks) {

            if (!lessonStart.isBefore(block.getTimeTo()) || !lessonEnd.isAfter(block.getTimeFrom())) {
